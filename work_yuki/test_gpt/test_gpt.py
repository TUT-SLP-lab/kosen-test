...
models = [ "gpt-4o-mini", "gpt-4-turbo", "gpt-4", "gpt-3.5-turbo", "gpt-4o"]
...
def call_chatgpt(text, models, reply, client):
    reply.append(call_langchain(text, models))
    

def call_true_or_false(question, model):
    output = []
    result = []
    result_list = [0, 0, 0]
    ...
    call_chatgpt(prompt_review, model, output, client)
    ...
    for i in range(10):
        call_chatgpt(prompt, models[np.random.randint(0, 2)], result, client)
        num = int(result[i])
        result_list[num - 1] += 1
...
def call_class(question, model):
    ...
    if 1 == int(result[0]):
        ...
        result = call_true_or_false(question, models[0])
