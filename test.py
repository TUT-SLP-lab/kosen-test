...
if text_tmp_answer == "1":
    ...
    print(text_question)
    #text_jud = f"「{question}」について、Web検索を使う場合は1、Web検索を使わない場合は2を出力"
    #text_jud_answer = gpt("質問変更2","gpt-4o",set_tmp,text_jud)
    #print(text_jud_answer)
    #if text_jud_answer == "1":
        # langchainを使用した回答
    text_answer = agents(set1,text_question)
    #elif text_jud_answer == "2":
        # モデルを使った回答生成
        # text_answer = gpt("通常の回答(3.5)", "gpt-3.5-turbo", set1, text_question)
    #else:
        #text_answer = "Web検索が必要か判断できません。プログラムを終了します"
    print(text_answer)
